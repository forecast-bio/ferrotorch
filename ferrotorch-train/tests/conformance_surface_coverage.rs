//! Conformance Layer 4 — strict per-item coverage gate for `ferrotorch-train`.
//!
...
// ---------------------------------------------------------------------------
// Surface inventory types (TOML — matches ferrotorch-train Layer 1)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Exclusions types (same TOML shape as ferrotorch-core and ferrotorch-mps)
// ---------------------------------------------------------------------------
...
fn every_public_item_has_a_conformance_reference_or_tracking_issue() {
    ...
    assert!(
        ...
        "no conformance test source files found in tests/. Expected at least \
         `tests/conformance_train.rs` to exist."
    );
    ...
    eprintln!("--- conformance surface coverage (ferrotorch-train) ---");
    eprintln!(
    ...
    assert!(
        uncovered.is_empty(),
        "{} ferrotorch-train public item(s) lack a conformance reference. \
