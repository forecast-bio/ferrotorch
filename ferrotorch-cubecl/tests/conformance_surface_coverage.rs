//! Conformance Layer 4 — strict per-item coverage gate for ferrotorch-cubecl.
//!
...
//! and scans the `tests/conformance_cubecl.rs` file for references to each item.
...
//!
//! This is the project's lock-in: no public API can be added to ferrotorch-cubecl
...
/// Read every `tests/conformance_cubecl*.rs` file and return their concatenated
/// source. The coverage check is a substring grep — an item is "covered" iff
/// its short identifier appears anywhere in any conformance test source.
fn read_conformance_test_sources() -> String {
    ...
    for entry in entries {
        ...
        // Include conformance_cubecl*.rs; exclude the surface gate itself.
        if !name.starts_with("conformance_cubecl") {
            continue;
        }
...
//
// For a path like `ferrotorch_cubecl::ops::portable_add` → short ident is
// `portable_add`. For a method path like `ferrotorch_cubecl::CubeDevice::ordinal`,
// the coverage key is `CubeDevice::ordinal`. For glob re-exports (`...::*`), we
...
/// Placeholder values rejected as `tracking_issue`. Mirrors other suite gates.
...
fn every_public_item_has_a_conformance_reference_or_tracking_issue() {
    ...
    assert!(
        ...
        "no conformance_cubecl*.rs source files found in tests/. \
         Expected at least conformance_cubecl.rs."
    );
    ...
    eprintln!("--- ferrotorch-cubecl conformance surface coverage ---");
    eprintln!(
    ...
    assert!(
        uncovered.is_empty(),
        "{} ferrotorch-cubecl public item(s) lack a conformance reference. \
         Add a test in tests/conformance_cubecl.rs that references the item \
