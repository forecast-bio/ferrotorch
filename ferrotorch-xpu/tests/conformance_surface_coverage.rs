//! Conformance Layer 4 — strict per-item coverage gate for `ferrotorch-xpu`.
//!
...
//!
//! Mirrors the pattern in `ferrotorch-core/tests/conformance_surface_coverage.rs`.
...
// ---------------------------------------------------------------------------
// Surface inventory types (TOML, not JSON — matches ferrotorch-xpu's Layer 1)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Exclusions types (same TOML shape as ferrotorch-core)
// ---------------------------------------------------------------------------
...
/// Read every `tests/conformance_*.rs` except this gate and the inventory
/// itself; return concatenated source.
...
// ---------------------------------------------------------------------------
// Coverage-key logic (mirrors ferrotorch-core pattern)
// ---------------------------------------------------------------------------
...
fn every_public_item_has_a_conformance_reference_or_tracking_issue() {
    ...
    // Validate exclusion entries before use.
    ...
    assert!(
        ...
        "no conformance test source files found in tests/. Expected at least \
         `tests/conformance_xpu.rs` to exist."
    );
    ...
    eprintln!("--- conformance surface coverage (ferrotorch-xpu) ---");
    eprintln!(
    ...
    assert!(
        uncovered.is_empty(),
        "{} ferrotorch-xpu public item(s) lack a conformance reference. \
