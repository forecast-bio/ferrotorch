...
pub fn contiguous_t<T: Float>(input: &Tensor<T>) -> FerrotorchResult<Tensor<T>> {
    if input.is_contiguous() {
        ...
        // For now, all tensors are contiguous, so this branch is unreachable.
        Ok(input.clone())
    }
}
...
pub fn split_t<T: Float>(
    ...
    for &split_size in split_sizes {
        ...
        for o in 0..outer {
            let src_start = o * total_along_dim * inner + offset;
        ...
        let t = Tensor::from_storage(TensorStorage::cpu(chunk_data), chunk_shape, false)?;
        results.push(t);
