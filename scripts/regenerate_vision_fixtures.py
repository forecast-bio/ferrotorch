...
Usage:
    ...
    python3 scripts/regenerate_vision_fixtures.py --models resnet50
...
Regenerate-before-test contract (for --models value-parity flow):
    The value-parity flow writes large binary artefacts (model weights as
    safetensors, plus deterministic input + reference output as raw float32
    .bin files) under ferrotorch-vision/tests/conformance/fixtures/. These
    artefacts are .gitignored and MUST be regenerated by running this script
    with `--models <name>` before invoking the corresponding `cargo test
    -p ferrotorch-vision --test conformance_vision_models` target. CI runs
    the same regenerate step as a build prerequisite. The on-disk fixture
    file `fixtures_value_parity.json` carries the schema (paths + sha256
    checksums) and IS committed; the binary blobs are not.
"""
...
# ---------------------------------------------------------------------------
# CLI: --models <name> selects the value-parity flow and short-circuits past
# the small-fixture builders below. Without --models, the historical
...
def _value_parity_main(model_names):
    ...
    For each name we:
      ...
      2. Capture state_dict, save as safetensors.
      3. Build a deterministic input tensor, save as raw little-endian f32
    ...
    payload = {
        "metadata": {
            ...
            "schema_version": 1,
            "description": (
                "Value-parity vision fixtures: every entry references binary "
                "artefacts (safetensors weights + raw little-endian f32 input "
                "and expected-output blobs) that are .gitignored. Run "
                "`python3 scripts/regenerate_vision_fixtures.py --models <name>` "
                "to materialise them locally before `cargo test "
                "-p ferrotorch-vision`."
            ),
        },
        "fixtures": merged,
...
def _emit_value_parity_descriptor(
    *,
    ...
    return {
        ...
        "output_dtype": "f32",
        "weights_path": f"fixtures/{weights_name}",
        "weights_sha256": weights_sha,
        "input_path": f"fixtures/{input_name}",
        "input_sha256": input_sha,
        "expected_output_path": f"fixtures/{output_name}",
...
def _write_f32_bin(path, tensor):
    """Save a float32 tensor as raw little-endian bytes (no header)."""
...
_parser.add_argument(
    ...
    nargs="+",
    default=None,
    help="Run only the value-parity flow for these models (e.g. resnet50). "
         "When omitted, regenerate the historical small-fixture JSON.",
)
...
if _args.help:
    ...
    sys.exit(0)
if _args.models:
    _value_parity_main(_args.models)
    sys.exit(0)
