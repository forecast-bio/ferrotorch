...
pub struct RAdam<T: Float> {
    ...
    state: HashMap<ParamKey, RAdamParamState>,
    /// Foreach (on-device) state. Used when `config.foreach == true`.
    foreach_state: HashMap<ParamKey, RAdamForeachState<T>>,
}
...
mod tests {
    ...
    fn test_radam_state_dict_roundtrip() {
        ...
        let key: String = RAdam::<f64>::param_key(0, 0).to_string();
