//! Conformance Layer 4 — strict per-item coverage gate for `ferrotorch-data`.
//!
//! Tracking issue: #838 (ferrotorch-data conformance suite).
//!
...
// ---------------------------------------------------------------------------
// Surface inventory types (TOML — matches ferrotorch-data Layer 1)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Coverage-key logic (mirrors ferrotorch-mps pattern)
// ---------------------------------------------------------------------------

/// Strip leading generic parameters from a path segment.
/// `"DataLoader <D>"` → `"DataLoader"`, `"BatchSampler <S>"` → `"BatchSampler"`.
fn strip_generics(seg: &str) -> &str {
...
fn coverage_keys(path: &str) -> Vec<String> {
    ...
    // unrelated symbols sharing the same short name.  Strip generic params
    // from the type segment so `"DataLoader <D>::drop_last"` becomes
    // `"DataLoader::drop_last"` and matches the test source.
    if segs.len() >= 3
        && segs[segs.len() - 2]
            .chars()
            .next()
    ...
    {
        let ty = strip_generics(segs[segs.len() - 2]);
...
fn every_public_item_has_a_conformance_reference_or_tracking_issue() {
    ...
    assert!(
        ...
        "no conformance test source files found in tests/. Expected at least \
         one `tests/conformance_*.rs` file to exist."
    );
    ...
    eprintln!("--- conformance surface coverage (ferrotorch-data) ---");
    eprintln!(
    ...
    assert!(
        uncovered.is_empty(),
        "{} ferrotorch-data public item(s) lack a conformance reference. \
