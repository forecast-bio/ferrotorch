...
impl<T: Float> RandomApply<T> {
    ...
    pub fn new(transforms: Vec<Box<dyn Transform<T>>>, p: f64) -> Self {
        assert!(
            (0.0..=1.0).contains(&p),
            "RandomApply: p must be in [0.0, 1.0], got {p}"
        );
        Self { transforms, p }
    }
}
...
impl<T: Float> RandomChoice<T> {
    ...
    pub fn new(transforms: Vec<Box<dyn Transform<T>>>) -> Self {
        assert!(
            !transforms.is_empty(),
            "RandomChoice: transforms list must not be empty"
        );
        Self { transforms }
    }
}
...
mod tests {
    ...
    fn test_random_apply_always() {
        ...
        let ra = RandomApply::<f64>::new(
            vec![Box::new(Normalize::<f64>::new(vec![1.0], vec![1.0]))],
            1.0,
        );
    ...
    fn test_random_apply_empty_transforms() {
        ...
        let ra = RandomApply::<f64>::new(vec![], 1.0);
        let out = ra.apply(t).unwrap();
