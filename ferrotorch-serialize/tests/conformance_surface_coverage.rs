//! Conformance Layer 4 — strict per-item coverage gate for `ferrotorch-serialize`.
//!
...
// ---------------------------------------------------------------------------
// Surface inventory types (TOML — matches ferrotorch-serialize Layer 1)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Exclusions types (same TOML shape as ferrotorch-mps and ferrotorch-train)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Coverage-key logic (mirrors ferrotorch-mps / ferrotorch-train pattern)
// ---------------------------------------------------------------------------
...
fn every_public_item_has_a_conformance_reference_or_tracking_issue() {
    ...
    assert!(
        ...
        "no conformance test source files found in tests/. Expected at least \
         `tests/conformance_serialize.rs` to exist."
    );
    ...
    eprintln!("--- conformance surface coverage (ferrotorch-serialize) ---");
    eprintln!(
    ...
    assert!(
        uncovered.is_empty(),
        "{} ferrotorch-serialize public item(s) lack a conformance reference. \
