//! Conformance Layer 4 — strict per-item coverage gate for `ferrotorch-distributions`.
//!
...
//!
//! Mirrors the pattern from `ferrotorch-vision/tests/conformance_surface_coverage.rs`.
...
fn coverage_keys(path: &str) -> Vec<String> {
    ...
    // For Type::method paths, use "Type::method" to avoid false positives on
    // short idents like `new` or `rate` that could match anything.
    if segs.len() >= 3
        && segs[segs.len() - 2]
            .chars()
            .next()
...
fn every_public_item_has_a_conformance_reference_or_tracking_issue() {
    ...
    assert!(
        ...
        "no conformance test source files found in tests/. Expected at least \
         `tests/conformance_distributions_continuous.rs` to exist."
    );
    ...
    eprintln!("--- conformance surface coverage (ferrotorch-distributions) ---");
    eprintln!(
    ...
    assert!(
        uncovered.is_empty(),
        "{} ferrotorch-distributions public item(s) lack a conformance reference. \
